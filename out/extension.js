...
const owaspScanner_1 = require("./scanners/owaspScanner");
const aiScanner_1 = require("./scanners/aiScanner");
const securityScore_1 = require("./analytics/securityScore");
const problemProvider_1 = require("./providers/problemProvider");
const codeActionProvider_1 = require("./providers/codeActionProvider");
const hoverProvider_1 = require("./providers/hoverProvider");
const cliScanner_1 = require("./cli/cliScanner");
const githubAction_1 = require("./integrations/githubAction");
function activate(context) {
    ...
    const owaspScanner = new owaspScanner_1.OWASPScanner();
    const aiScanner = new aiScanner_1.AIScanner();
    const securityScore = new securityScore_1.SecurityScoreCalculator();
    const cliScanner = new cliScanner_1.CLIScanner();
    const githubAction = new githubAction_1.GitHubAction();
    ...
    context.subscriptions.push(scanDocument, scanWorkspace, scanDependencies, scanSecrets, owaspScan, securityScoreCmd, cliScan);
    // Register providers
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider('*', codeActionProvider), vscode.languages.registerHoverProvider('*', hoverProvider), vscode.window.registerTreeDataProvider('sec4devProblems', problemProvider));
    // Real-time scanning
        ...
        const gitListener = vscode.workspace.onDidChangeConfiguration(async (event) => {
            if (event.affectsConfiguration('git')) {
                ...
                if (score.score < config.get('securityScoreThreshold', 70)) {
                    vscode.window.showWarningMessage(`Security score is ${score.score}/100. Consider fixing issues before committing.`);
                }
            }
        });
...
function scanDocumentForEval(document) {
...
async function comprehensiveScan(document) {
    const config = vscode.workspace.getConfiguration('sec4dev');
    const result = {
        evalCount: 0,
        suspiciousPatterns: [],
...
async function scanWorkspaceComprehensive() {
    const results = [];
    const files = await vscode.workspace.findFiles('**/*.{js,ts,jsx,tsx,py,php,json,yaml,yml,xml}', '**/node_modules/**');
...
function showResults(result) {
    const panel = vscode.window.createWebviewPanel('sec4devResults', 'Sec4Dev Security Results', vscode.ViewColumn.One, {});
...
function showWorkspaceResults(results) {
    const panel = vscode.window.createWebviewPanel('sec4devWorkspaceResults', 'Sec4Dev Workspace Security Scan', vscode.ViewColumn.One, {});
...
function showRealTimeWarning(result) {
    ...
    if (result.evalCount > 0 || highRiskCount > 0) {
        vscode.window.showWarningMessage(`🚨 Sec4Dev Security Alert: ${result.evalCount} eval() functions and ${highRiskCount} high-risk patterns detected!`, 'View Details').then(selection => {
...
function showDependencyResults(vulnerabilities) {
    const panel = vscode.window.createWebviewPanel('sec4devDependencies', 'Sec4Dev Dependency Vulnerabilities', vscode.ViewColumn.One, {});
    panel.webview.html = `
    <!DOCTYPE html>
    <html>
    <head>
...
function showSecretResults(secrets) {
    const panel = vscode.window.createWebviewPanel('sec4devSecrets', 'Sec4Dev Secrets Detection', vscode.ViewColumn.One, {});
    panel.webview.html = `
    <!DOCTYPE html>
    <html>
    <head>
...
function showOWASPResults(issues) {
    const panel = vscode.window.createWebviewPanel('sec4devOWASP', 'Sec4Dev OWASP Top 10 Scan', vscode.ViewColumn.One, {});
    panel.webview.html = `
    <!DOCTYPE html>
    <html>
    <head>
...
function showSecurityScoreDashboard(score) {
    const panel = vscode.window.createWebviewPanel('sec4devScore', 'Sec4Dev Security Score Dashboard', vscode.ViewColumn.One, {});
    panel.webview.html = `
    <!DOCTYPE html>
    <html>
    <head>
...
function showCLIResults(result) {
    const panel = vscode.window.createWebviewPanel('sec4devCLI', 'Sec4Dev CLI Scan Results', vscode.ViewColumn.One, {});
    panel.webview.html = `
    <!DOCTYPE html>
    <html>
    <head>
